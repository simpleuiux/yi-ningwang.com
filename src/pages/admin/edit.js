...
const EditArticlePage = ({ location, data }) => {
  ...
  const searchParams = typeof window !== "undefined" && location ? new URLSearchParams(location.search) : null
  ...
  const fetchArticle = async () => {
    ...
    if (data && data.allMarkdownRemark && data.allMarkdownRemark.edges) {
      ...
      if (foundArticle) {
        ...
        try {
          const response = await fetch(`/api/article-content/${encodeURIComponent(slug)}`)
    ...
    try {
      ...
      const response = await fetch(`/.netlify/functions/get-article?slug=${encodeURIComponent(slug)}`, {
        headers: {
